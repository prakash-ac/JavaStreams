...
public class ProductBigDataQuestions {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Number of products \t\t\t\t\t73547: " + numberOfProducts);
        ...
        System.out.println("Number of chemicals in all products \t\t\t81789: " + numberChemicalsInAllProducts);
       ...
       System.out.println("It appears in this many products \t\t\t\t67896: " + (chemicalProductMap.get("Titanium dioxide")).size());
    ...
    public static int q1() {
    	...
    	return productList.stream()
    			.mapToInt(product -> product.getNumberOfChemicals()).sum(); 
    }
    public static int q2() {
        // QUESTION 2: How many different companies are in the dataset?
    	return (int)productList.stream()
    			.map(product -> product.getCompany())
    			.distinct().count();
    }
    ...
    public static Map<String, List<Product>> q4() {
        ...
        // Hint: use Collectors.groupingBy!
    	return productList.stream()
    			.collect(Collectors.groupingBy(Product::getCompany)); //product -> product.getCompany()
    	
    }
    public static String q5(Map<String, List<Product>> companyProductMap) {
    	// QUESTION 5: Use the map you created above. Which company has the most products?
        // Hint: Use .max(Comparator) and define the Comparator to compare companies based on size of their lists.
    	return companyProductMap.keySet().stream()
    			.max((key1, key2) -> Integer.compare(companyProductMap.get(key1).size(), companyProductMap.get(key2).size())).get();
    }
    public static void q6( Map<String, List<Product>> chemicalProductMap) {
    	...
    	productList.stream()
    		.forEach(product -> {
    			product.getChemicals().stream().forEach(chemicalName ->{
    					List<Product> tempProductList = chemicalProductMap.get(chemicalName);
    					tempProductList.add(product);
    					chemicalProductMap.put(chemicalName, tempProductList);
    			});
    ...
    public static String q7() {
        // QUESTION 7: Which chemical appears in the most products?
        // Hint: use max(Comparator) again. Define your comparator to compare chemical names based on the size of the list of products.
    	return chemicalProductMap.keySet().stream()
    			.max((key1, key2) -> 
    			Integer.compare(chemicalProductMap.get(key1).size(), chemicalProductMap.get(key2).size())).get();
    }
    public static List<Product> createList() {

        String line = "";
        String fileName = "/users/prakash/eclipse-workspace/streams/bin/ChemicalData.csv";
